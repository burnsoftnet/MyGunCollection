...
namespace BSMyGunCollection.UnitTest.UI.Collection
{
    ...
    public class ViewWindowTest
    {
        ...
        public void CollectionViewTest()
        {
            ...
            try
            {
                List<BatchCommandList> value = _ga.RunBatchCommands(Command.Helpers.UI.Collection.ViewWindow.RunTest(_firearmToView, true, false, false), out _errOut);
        ...
        public void ChangeToCompetitionGunInViewTest()
        {
            ...
            try
            {
                List<BatchCommandList> value = _ga.RunBatchCommands(Command.Helpers.UI.Collection.ViewWindow.RunTest(_firearmToView, false, true, false), out _errOut);
                if (_errOut.Length > 0) throw new Exception(_errOut);
                DumpResults(value);
                bans = _ga.AllTestsPassed(value);
        ...
        public void ChangeToNonLethalGunInViewTest()
        {
            ...
            try
            {
                List<BatchCommandList> value = _ga.RunBatchCommands(Command.Helpers.UI.Collection.ViewWindow.RunTest(_firearmToSetAsNonLethal, false, false, true), out _errOut);
                if (_errOut.Length > 0) throw new Exception(_errOut);
                DumpResults(value);
                bans = _ga.AllTestsPassed(value);
